...
const GAME_STATE = { tries: 0, matches: 0 };
let numOfCards = 0;
let cardsClicked = 1;
let prevCardNum = 0;
let timeoutID;
...
const updateGameState = () => {
    if (GAME_STATE.matches === 0 && GAME_STATE.tries === 0)
        document.querySelector('.gameState').innerHTML = 'Find all the pairs!';
    else
        document.querySelector('.gameState').innerHTML = `You found ${GAME_STATE.matches} out of ${numOfCards / 2} pairs with ${GAME_STATE.tries} tries`;
};
